...
const ShoeModel = ({modelRef, groupRef,url, rotation, setModelRayData}) => {
    // GLOBAL STATE
    const {
        animation,
        decals,
    ...
    const {decalsData, addDecalData} = useDecalStore()
    ...
    return (<a.group ref={groupRef} rotation={rotation} castShadow scale={scale}>
        <mesh
            ...
            castShadow={true}
            geometry={nodes.Rest_2.geometry}
            scale={s}
        >
        ...
        <mesh scale={s} name="Rest_2" geometry={nodes.Rest_1.geometry} material-color={modelColor}/>
...
useGLTF.preload('/models/shoe/shoe.glb')